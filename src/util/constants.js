// export const PARKING_SERVER_URL = 'http://207.194.227.246:3000/api/parkingSpots';
export const PARKING_SERVER_URL = 'http://192.168.1.82:3000/api/parkingSpots';
export const LATITUDE_DELTA = 0.010;

// Location permission constants
export const PERMISSION_REQ_TITLE = 'Parking App Location Permission';
export const PERMISSION_REQ_MSG = 'Parking App needs access to your location to locate nearby parking.';
export const CALLOUT_TEXT = 'Parking Meter';